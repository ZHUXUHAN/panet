...
from .vgg import Encoder, Feature_Header, FPN_Encoder
from .mobile import mobilenetv3_large, mobilenetv3_small
...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        #     ('backbone', FPN_Encoder(in_channels, self.pretrained_path)), ]))
        #
        # self.encoder = nn.Sequential(OrderedDict([
        #     ('backbone', Encoder(in_channels, self.pretrained_path)), ]))
        # self.feature_conv = Feature_Header(512)
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        if type(img_fts) != list:
            ...
            ###### Compute loss ######
            ######## 0
            fts_size = img_fts[0].shape[-2:]
            supp_fts = img_fts[0][:n_ways * n_shots * batch_size].view(
                n_ways, n_shots, batch_size, -1, *fts_size)  # Wa x Sh x B x C x H' x W'
            qry_fts = img_fts[0][n_ways * n_shots * batch_size:].view(
                n_queries, batch_size, -1, *fts_size)  # N x B x C x H' x W'
            ...
            for epi in range(batch_size):
                ...
                dist = [self.calDist(qry_fts[:, epi], prototype) for prototype in prototypes]
                pred = torch.stack(dist, dim=1)  # N x (1 + Wa) x H' x W'
                outputs_0.append(F.interpolate(pred, size=img_size, mode='bilinear'))

                ###### Prototype alignment loss ######
                if self.config['align'] and self.training:
                    align_loss_epi = self.alignLoss(qry_fts[:, epi], pred, supp_fts[:, :, epi],
                                                    fore_mask[:, :, epi], back_mask[:, :, epi])
                    align_loss_0 += align_loss_epi

            outputs_0 = torch.stack(outputs_0, dim=1)  # N x B x (1 + Wa) x H x W
            outputs_0 = outputs_0.view(-1, *outputs_0.shape[2:])

            ########1
            # for img_fts in img_fts_list:
            fts_size = img_fts[1].shape[-2:]

            supp_fts = img_fts[1][:n_ways * n_shots * batch_size].view(
                n_ways, n_shots, batch_size, -1, *fts_size)  # Wa x Sh x B x C x H' x W'
            qry_fts = img_fts[1][n_ways * n_shots * batch_size:].view(
                n_queries, batch_size, -1, *fts_size)  # N x B x C x H' x W'
            ...
            for epi in range(batch_size):
                ...
                dist = [self.calDist(qry_fts[:, epi], prototype) for prototype in prototypes]
                pred = torch.stack(dist, dim=1)  # N x (1 + Wa) x H' x W'
                outputs_1.append(F.interpolate(pred, size=img_size, mode='bilinear'))

                ###### Prototype alignment loss ######
                if self.config['align'] and self.training:
                    align_loss_epi = self.alignLoss(qry_fts[:, epi], pred, supp_fts[:, :, epi],
                                                    fore_mask[:, :, epi], back_mask[:, :, epi])
                    align_loss_1 += align_loss_epi

            outputs_1 = torch.stack(outputs_1, dim=1)  # N x B x (1 + Wa) x H x W
            outputs_1 = outputs_1.view(-1, *outputs_1.shape[2:])

            output = outputs_0+outputs_1
            align_loss = (align_loss_0+align_loss_1)/2
            # outputs_list.append(output)
            # align_loss_list.append(align_loss / batch_size)
            # output = outputs_list[0]+outputs_list[1]
            # align_loss = sum(align_loss_list)/len(align_loss_list)
            return output, align_loss / batch_size
